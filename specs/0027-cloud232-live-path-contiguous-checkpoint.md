...
| `events_since_checkpoint` | `Batched` `batch_size` threshold | `update()` and `record_processed()` |
...
3. The contiguous branch keeps direct field assignment (`p.global_sequence = new_contiguous; p.event_id = checkpoint_event_id`) and does **not** call `update()`. Calling `update()` here would double-count: each processed event is already counted once by `record_processed` at its per-event site, so a second increment in the advance branch would trip the `batch_size` threshold early. The paired id for the next batch's eager seed is kept current by a separate assignment `state.contiguous_event_id = checkpoint_event_id` alongside the `contiguous_checkpoint` advance.
...
**Which existing constructor becomes which, stated outright** because the design turns on not swapping them: `PendingCheckpoint::new` **becomes the advance constructor** (born publishable), since its existing call sites are the advance path and the two per-event sites that are being replaced by `record_processed` anyway. The eager seed gets the **new** constructor. That is the only assignment consistent with "keep the `None` arm as the publishable one" in §3.4.
...
>
> So the test-infrastructure phase must add a `table: &str` parameter to both helpers, and the isolated table's hole must be claimed through **that table's** `DEFAULT nextval` so it burns the private sequence. This is real work, not a detail, and §9.2's revert check is the only gate that would catch it being skipped.
>
> **Third caller, easy to miss:** `create_sequence_gap` also calls `insert_committed_event` (and does its own `epoch_events` rollback insert). It is the fixture behind `test_rolled_back_gap_fence_clears_without_record` and `test_pinned_gap_resolves_via_backstop`, so changing the helper signature without updating it is a mid-phase compile error, i.e. exactly the thing that breaks "each phase leaves the suite green".
...
- **Harden `test_synchronous_checkpoint_still_works` the same way. ADDED after review, and it closes the most dangerous gap in this plan.** Every other positive control here is `Batched`-only, yet the change most likely to break cadence — moving the flush call out of the advance branch — affects `Synchronous` too, and `Synchronous` is the default. Without this there is **no control that the default mode still advances its checkpoint per batch**. Exact-value, isolated table. Store **two or three** events rather than the current one and assert the final sequence: a single-event version still catches a total failure to advance (the checkpoint would be `None`), but proves nothing about cadence across batches, and the extra events cost nothing. (R3)
...
**Post-P2 review finding:** `test_in_flight_transaction_gap_is_held` intermittently fails in full-suite runs (never in 10 isolated P2 runs; one failure in 15 combined runs). The failure is on the delivery assertion after commit (2s sleep), not on any checkpoint assertion. P2 cannot structurally cause this: the batch query lower bound is `min(subscriber_states.contiguous_checkpoint)` — the in-memory prefix driven by `advance_contiguous_checkpoint`, which P2 does not touch. This is the pre-existing load-sensitivity this section flags. **Recommended follow-up (out of this spec's scope):** give this test an isolated table and replace the bare post-commit sleep with a poll-with-timeout on delivery, matching the pattern used by the newer tests added in P1.

## 8. Phasing

Each phase is independently committable and leaves the suite green. Positive controls land in P1, i.e. no later than the phase that changes flushing behaviour.

> **Why there is no standalone "add the accounting primitives" phase.** An earlier draft opened with one, on the theory that adding `record_processed` and the publishable predicate with no callers is a free, behaviour-neutral commit. It is not: acceptance criterion 3 runs `cargo clippy --all-targets`, which builds the lib target **without** `cfg(test)`, and in that build a `pub(crate)` item with zero callers is `dead_code`, which `-D warnings` promotes to an error. The `#[cfg(test)]` unit tests do not count as callers there. So that phase would have failed its own gate. The primitives now land **with** their call sites, in P2.

- **P1 — test infrastructure and positive controls.** Convert `test_batched_checkpoint_flushes_at_batch_size` / `_at_max_delay` / `test_synchronous_checkpoint_still_works` to isolated-table exact-value assertions, and add the positive controls of §7.3 (the backstop control live, the fence-cleared one `#[ignore]`d). Add the integration-test copy of `cu_isolated_events_table` with its `setup_trigger`-before-`start_listener` wiring, **the `table` parameter on `claim_hole_uncommitted`, `insert_committed_event` and their caller `create_sequence_gap`, and the `PgEventStore::with_table` conversion** (§7.2 prerequisite), since P2 and P3 both need all of it. `test(pg): exact-value and no-stall controls for live checkpointing`.
  **What P1 does and does not prove.** An earlier draft claimed these "establish the baseline the next phase must not break". Overstated: pre-fix, `test_backstop_hole_still_advances_checkpoint`'s sequence assertion is satisfied **by the bug itself**, since max-seen already leaks a value at or above the target. Pre-fix these are trivially green and their real value is as a **post-fix tripwire**: once P2 lands, a version that stops the leak by never advancing turns them red. The ordering is still right for exactly the stated reason — the tripwire must already exist when the behaviour changes — but do not mistake a pre-fix pass for evidence of anything.
- **P2 — the fix, primitives included.** In `checkpoint.rs`: `PendingCheckpoint::record_processed`, the `seed_sequence` field, the two named constructors (§4 Q1) and the publishable predicate, with their unit tests. In `mod.rs`/`subscriber_state.rs`: Q1's `SubscriberState.contiguous_event_id` + seeding `SELECT`; both per-event sites switch to `record_processed` with eager seeding; the contiguous branch keeps direct field assignment (NOT `update()` — see §3.2 point 3: `update()` would double-count since `record_processed` already counted each event once); move `try_flush_pending_checkpoint` out of the advance branch under `!replay_always`; `cached_checkpoint` sourced only from `local_cache`; `debug_assert`; publishable predicate applied inside the `take_if` closure and inside both bulk flushers' selection. `fix(pg): publish only contiguous checkpoints from the live listener path`.
- **P3 — regression tests.** The three §7.2 tests plus the `ReplayAlways` guard. Land after P2 so they are green on commit; verify they fail against P2 reverted (§9.2). `test(pg): pin the live path against publishing above a held hole`.
- **P4 — hygiene and docs.** CHANGELOG `### Fixed` entry per §6, **scoped as §6 specifies** (do not claim a steady-state change on the default mode); rustdoc on `record_processed`, the new `SubscriberState` field, `new_with_event_id`, **both `PendingCheckpoint` constructors and the publishable predicate** (all new `pub(crate)` items, which acceptance criterion 5 requires); update `flush_checkpoint`'s hazard rustdoc to cite spec 0027 R1 alongside 0026 R1/R2; mark this spec Implemented with a §10 summary in 0026's style. `docs(pg): record the live-path contiguous checkpoint fix`.
...
```json
{
  ...
  "phases": [
    {
      "phase": 1,
      "focus": "epoch_pg/tests: test infrastructure and positive controls, all passing pre-fix. Add an integration-test copy of cu_isolated_events_table (CREATE TABLE LIKE + own sequence, setup_trigger BEFORE start_listener, Uuid-unique channel). PREREQUISITE, do not skip: add a table parameter to claim_hole_uncommitted, insert_committed_event AND their caller create_sequence_gap (the first two hardcode INSERT INTO epoch_events, so on an isolated table the hole is burned on the WRONG sequence and every later regression test passes vacuously; create_sequence_gap must be updated in the same commit or the suite will not compile). Convert event writes to PgEventStore::with_table or raw inserts, because PgEventStore::new hardcodes epoch_events so a bus pointed at an isolated table would receive nothing and hang. Convert test_batched_checkpoint_flushes_at_batch_size, _at_max_delay and test_synchronous_checkpoint_still_works to exact-value assertions on the isolated table, storing 2-3 events in the Synchronous one rather than 1. Add test_backstop_hole_still_advances_checkpoint (snapshot_fencing false, gap_timeout 500ms, zero load sensitivity, MANDATORY). Add test_fence_cleared_hole_still_advances_checkpoint as #[ignore] with a doc comment saying what it proves and to run it on a quiet instance (instance-wide xmin makes it flaky; the backstop control and the existing test_rolled_back_gap_fence_clears_without_record already cover the fence route).",
      "effort": "L",
      "difficulty": "hard",
    ...
    {
      "phase": 2,
      "focus": "epoch_pg/src/event_bus: the fix, including the accounting primitives it calls. In checkpoint.rs add PendingCheckpoint::record_processed (bumps events_since_checkpoint ONLY, leaving global_sequence/event_id/first_event_time untouched), a seed_sequence field, TWO named constructors, and the publishable predicate global_sequence > seed_sequence, with unit tests. CRITICAL: the existing PendingCheckpoint::new BECOMES the advance constructor (sets seed_sequence below its position, born PUBLISHABLE); the NEW constructor is the eager seed (sets seed_sequence to its own global_sequence, born UNPUBLISHABLE). Swapping these either makes the fix do nothing or freezes the checkpoint forever. These primitives must land in THIS phase, not earlier: with no callers they are dead_code, and cargo clippy --all-targets -D warnings would fail. Then: add SubscriberState.contiguous_event_id (plain Uuid) and SubscriberState::new_with_event_id, keeping the existing new() for its 21 call sites; add last_event_id to the start_listener seeding SELECT as (i64, Option<Uuid>) mapping None to nil. In process_subscriber_for_batch: both per-event sites call record_processed instead of update, seeding the PendingCheckpoint eagerly from state.contiguous_checkpoint and its paired id; the contiguous branch KEEPS direct field assignment (NOT update() — update() would double-count because record_processed already counted each processed event once at its per-event site; direct assignment moves the position without bumping events_since_checkpoint); KEEP the None arm, which uses the publishable advance constructor; move the try_flush_pending_checkpoint call OUT of the 'new_contiguous > contiguous_before' branch so it runs unconditionally under a !replay_always guard; set cached_checkpoint ONLY from local_cache, deleting the unconditional cached_checkpoint = Some(new_contiguous) on advance; add debug_assert!(pending <= state.contiguous_checkpoint) immediately before the flush. Apply the publishable predicate INSIDE the take_if closure in try_flush_pending_checkpoint and inside the selection/filter in flush_expired_checkpoints and flush_all_pending_checkpoints -- applying it after the take_if or after the map remove silently DESTROYS the pending and loses first_event_time. Do not merge the two advancers; do not touch advance_catchup_prefix or the public update_checkpoint.",
      "effort": "L",
      "difficulty": "hard",
    ...
    {
      "phase": 3,
      "focus": "epoch_pg/tests: regression tests that must FAIL with phase 2 reverted. test_live_shutdown_does_not_publish_above_held_hole (PRIMARY; default Synchronous, commented as such; also asserts the persisted last_event_id equals the below-hole event id, the only DB-level R5 check). test_live_batched_flush_does_not_publish_above_held_hole (Batched batch_size 1000 / max_delay_ms 300; ONE unavoidable fixed sleep >= 2x the hard-coded 1s flush_interval, taken AFTER delivery is verified, with a comment forbidding conversion to a poll loop). test_live_deser_skip_above_hole_does_not_publish_above_hole (raw inserts whose data is VALID JSON that is not a known event variant, e.g. {\"NoSuchVariant\":{}} -- not garbage bytes, which jsonb rejects at INSERT before deserialization is ever attempted; the delivery poll is impossible here since the deser branch continues, so captured_logs_contain_since on the WARN is its SUBSTITUTE signal, and the test must state its CheckpointMode and flush trigger explicitly). All on the isolated table, gap_timeout 30s, exact-equality against a committed below-hole sequence, observe-then-release-then-assert ordering. Plus a ReplayAlways guard. Verify each fails with phase 2 stashed.",
      "effort": "M",
      "difficulty": "hard",
...
1. New tests pass; existing gap-fence and gap-timeout tests pass, modulo the deliberate hardenings in P1.
2. Reverting P2 (the fix) makes §7.2 tests 1–3 fail — verified **mechanically by stashing**, not by inspection.
...
- **The seed predicate makes checkpoint catch-up slower after a lost outcome. Accepted, and it is the one real cost of the §4 Q1 amendment.** Normally `seed == persisted` holds, because a pending is cleared only by a *successful* flush (`take_if` retains it when the threshold is unmet and re-inserts it on error), so the next batch re-seeds from a `contiguous_checkpoint` that equals what was just written. The exception is the lost-outcome path described below: if a task never returns, its pending is dropped from the map while `state.contiguous_checkpoint` may already have advanced. The next batch then seeds at a position *above* the persisted value, and the predicate correctly refuses to publish it, so the persisted checkpoint stays stale until the prefix advances again. Under the superseded `> 0` rule the seed would have been published and caught the persisted value up.
  This is a freshness regression, not a correctness one: the §3.3 invariant explicitly permits the persisted value to lag, and the consequence of lagging is redelivery, which the bus is at-least-once about anyway. Recorded because it is a deliberate trade, not an oversight.
- **The `cached_checkpoint` sourcing change (P2) has no dedicated test.** It rides in the same phase as the flush move and changes what the listener cache is fed: under the new predicate an unpublishable pending produces no `local_cache` entry, so `cached_checkpoint` becomes `None` where it was `Some(new_contiguous)`. That is the intended semantics and the map currently has no readers, so nothing should observe it — but "nothing should observe it" is an argument, not a test. If this plan produces one surprise, this is the most likely source.
