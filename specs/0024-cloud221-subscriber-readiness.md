...
- **OQ-4 (resolved, spec 0026/CLOUD-226):** ~~the reused `subscribe()` catch-up advances a linear `PendingCheckpoint` with **no** gap fence, so over `1,2,3,[gap 4],5` it flushes `checkpoint = 5` and a late event 4 is skipped. Pre-existing hazard, merely propagated into listener startup here; fix under a separate ticket.~~ Fixed in spec 0026: both `catch_up_from_checkpoint` and the `subscribe()` buffer-drain pass now persist only the highest contiguous prefix from the starting checkpoint, never advancing the persisted value past a hole.
