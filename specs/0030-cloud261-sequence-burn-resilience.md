...
Traceability (per 0028's house pattern, `specs/0028-cloud216-fail-closed-subscriber-semantics.md:443`): R1→T0, R2→T1, R3→T2, R4→T3+T7, R5→T4, R6→T5, R8→T6, R9→T8, R10→T11, R11→T13+T14, R12→T12 + the missing-counter-row pin, R13→T9+T10, R14→T15. **R7 and R15 are doc-only** (rustdoc/CHANGELOG obligations, verified by review in P5/P8); every other requirement traces to a numbered test.

Shipped test names (reconciled in P8 against `epoch_pg/tests/pgeventbus_integration_tests.rs`, and `epoch_core/src/projection.rs` for T0): T0 `projection.rs` gap-policy forwarding-chain test; T1 `test_subscribe_gap_policy_guardrail_skip_after_backstop_replay_always_only`; T2 `test_skip_after_backstop_advances_hwm_past_unproven_gap`; T3 `test_skip_after_backstop_audited_row_precedes_hwm_advance`; T4 `test_late_materialization_detected_fires_rebuild_once`; T5 the `GapPolicy::Halt` default-unchanged regression pin; T6 `test_release_halt_warn_does_not_promise_resume_for_replay_always`; T7 `test_skip_after_backstop_ledger_failure_withholds_skip_then_retries`; T8 `test_readiness_reports_caught_up_across_skipped_hole`; T9 `test_per_txn_counter_rollback_burns_nothing`; T10 `test_per_txn_counter_crash_burns_nothing`; T11 `test_nextval_rollback_still_burns_and_touches_no_counter` (plus `test_nextval_allocation_mode_touches_no_counter_row` from P6); T12 `test_per_txn_counter_allocates_once_per_transaction`; T13 `test_per_txn_counter_construction_reseeds_counter_row`; T14 `test_opt_in_transition_assigns_above_every_previous_value`; T15 `test_per_txn_counter_concurrent_writers_are_contiguous`. The P7 review-finding pin is `test_per_txn_counter_missing_counter_row_fails_the_insert` (no silent `nextval` fallback); its in-file comment label `T7b-pin` is unfortunate — `T7b` already names an unrelated CLOUD-227 test group in the same file, so cite the pin by function name, not by label.
...
- **P3 — `SkipAfterBackstop` in the pure resolver + audited skip.** T2, T3, T7 red → green. Thread `gap_policy` into `advance_contiguous_checkpoint`; `FailClosed`+`SkipAfterBackstop` takes the `TimeoutBackstop` advance path (`subscriber_state.rs:376-386`) **after the confirmed ledger insert** (record-then-advance, §3.1 — the resolver offers the skip unapplied, the caller confirms the ledger and then applies it); the ledger INSERT is made synchronous-confirmed for this path while the fail-open path keeps its fire-and-forget recorder; the existing WARN + `on_gap_timeout` + HWM routing (`mod.rs:558-714`) run unchanged. (R3, R4)
